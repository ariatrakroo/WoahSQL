...
#QUERY 1
#identifying which demographics transact most frequently
...
#QUERY 4
WITH ranking AS (
SELECT ProductCategory, Age, AnnualIncome, NumberOfPurchases,
       DENSE_RANK() OVER (PARTITION BY ProductCategory ORDER BY NumberOfPurchases DESC) AS purchase_rank
FROM Customer
)
SELECT ProductCategory, Age, AnnualIncome, NumberOfPurchases, purchase_rank
FROM ranking
...
SELECT NumberOfPurchases, TimeSpentOnWebsite, TimeSpentOnWebsite/NumberOfPurchases AS turnover,
CASE
